...
export default function GamesIndex() {
  ...
  useEffect(() => {
    const handleExitedGames = async () => {
      ...
      if (exitedFlappyGame) {
        ...
        await endSession(); // Save session to DB
        resetCurrentScore(); // Reset only after session saved
  ...
  const handleSelectGame = (
    ...
    // Reset exit flags before starting new game
    setExitedFlappyGame(false);
    setExitedSnake(false);

    // Start a new session for the selected game
    startChildSession(currentChildId, "game");
    setSessionDetails(`Playing ${gameName}`);

    // Navigate safely to the chosen game
    router.push(route);
  };
