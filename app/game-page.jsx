...
import * as Haptics from 'expo-haptics'; // why: light tactile confirmation improves perceived responsiveness
...
const GAMES = [
  ...
  },
  {
    key: 'quiz',
    title: 'Quiz',
    image: require('@/assets/images/saftey-sign.png'),
    onPressType: 'route',
    route: './QuizScreen',
...
// --- Card ---
const GameCard = memo(function GameCard({ title, image, onPress, disabled, tint }) {
    ...
    >
      <Animated.View style={{ flex: 1, transform: [{ scale }] }}>
        <Pressable
          ...
          accessibilityHint={disabled ? undefined : `Opens ${title}`}
          testID={`game-card-${title.toLowerCase()}`}
          hitSlop={10}
        >
...
const GamePage = memo(() => {
  ...
  const handleSelect = useCallback(
    (item) => {
      ...
      else if (item.onPressType === 'route' && item.route) router.push(item.route);
    },
    [router]
  );
  ...
  if (selectedGame === 'flappy') {
    return (
      <SafeAreaView style={{ flex: 1 }}>
        <HeaderInline onBack={() => setSelectedGame(null)} title="Flappy" />
        <Flappy />
      </SafeAreaView>
    );
  }
  if (selectedGame === 'snake') {
    return (
      <SafeAreaView style={{ flex: 1 }}>
        <HeaderInline onBack={() => setSelectedGame(null)} title="Snake" />
        <Snake />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={[styles.backContainer, { top: insets.top + hp('1%') }]}>
        <Pressable
          ...
          onPress={() =>
            navigation.canGoBack() ? navigation.goBack() : navigation.navigate('LogInPage')
          }
          accessibilityRole="button"
      ...
      <FlatList
        ...
        getItemLayout={(_, index) => {
          const itemHeight = CARD_H + hp('2.2%'); // why: include row spacing to align measurements with layout
          const row = Math.floor(index / 2);
          return { length: itemHeight, offset: row * itemHeight, index };
        }}
        ListFooterComponent={<View style={{ height: Math.max(insets.bottom, hp('2%')) }} />}
      />
    </SafeAreaView>
  );
});

const HeaderInline = memo(function HeaderInline({ onBack, title }) {
  const insets = useSafeAreaInsets();
  return (
...
const styles = StyleSheet.create({
  ...
  // // badge
  // badgeCorner: { position: 'absolute', top: 8, right: 8 },
  // badge: {
  //   flexDirection: 'row',
  //   alignItems: 'center',
  //   borderColor: '#000',
