...
const STATUS_BAR_PAD = Platform.OS === "android" ? 30 : 12;
const NAV_BAR_PAD = Platform.OS === "android" ? 20 : 10;
...
export default function FruitNinjaGame() {
  ...
  const sliceFruit = useCallback((fruit: Fruit) => {
    ...
    setFruits((prev) =>
      prev.map((f) => (f.id === fruit.id ? { ...f, sliced: true } : f))
    );
  ...
  useEffect(() => {
    ...
    const scheduleNext = () => {
      ...
      const ms = Math.max(
        MIN_SPAWN_MS,
        Math.round(BASE_SPAWN_MS - (BASE_SPAWN_MS - MIN_SPAWN_MS) * progress)
      );
  ...
  const makePath = (
    ...
    if (pattern === "bottomUp") {
      ...
      return {
        ...
        p1: { x: startX + wobble, y: peakY },
        p2: { x: startX + wobble * 1.4, y: OFFSCREEN_Y },
        riseMs,
        holdMs,
        fallMs,
      };
  ...
  const spawnFruit = () => {
    ...
    const sub = progress.addListener(({ value }) => {
      ...
      const drift =
        Math.sin(value * Math.PI * 2 * fruit.driftFreq) * fruit.driftAmp;
  ...
  return (
    <SafeAreaView style={styles.safeContainer}>
      ...
      {/* Background emojis */}
      <Text style={[styles.bgEmoji, { left: 16, top: 90, opacity: 0.14 }]}>
        ☁️
      </Text>
      <Text style={[styles.bgEmoji, { right: 26, top: 160, opacity: 0.12 }]}>
        ☁️
      </Text>
      <Text style={[styles.bgEmoji, { left: 42, bottom: 160, opacity: 0.1 }]}>
        🌈
      </Text>
      ...
      <View style={styles.hudRow}>
        <View
          style={[
            styles.pill,
            { backgroundColor: "#E6F7FF", borderColor: "#98D5FF" },
          ]}
        >
          <Text style={styles.pillText}>⏱ {timeLeft}s</Text>
        </View>
        <View
          style={[
            styles.pill,
            { backgroundColor: "#E8F8EF", borderColor: "#9AD9B9" },
          ]}
        >
        ...
        <View
          style={[
            styles.pill,
            { backgroundColor: "#FFF6E6", borderColor: "#FFD28C" },
          ]}
        >
          <Text style={styles.pillText}>⭐ {hiScore}</Text>
        </View>
      </View>
      ...
      {fruits.map((fruit) => {
        ...
        const rotateDeg = fruit.progress.interpolate({
          inputRange: [0, 1],
          ...
          <Animated.Text
            ...
            style={[
              styles.fruit,
              {
                ...
                top: fruit.y,
                opacity: fruit.fade,
                transform: [{ rotate: rotateDeg }, { scale: fruit.scale }],
              },
            ]}
          >
            {fruit.emoji}
      ...
      {gameOver && (
        <View style={[styles.overlay, { zIndex: 1000 }]} pointerEvents="auto">
          <View style={styles.modal}>
            ...
            <Pressable onPress={resetGame} style={styles.btn}>
      ...
      >
        {angles.map((a, i) => {
          const tx = progress.interpolate({
            inputRange: [0, 1],
            outputRange: [0, Math.cos(a) * distances[i]],
          });
          const ty = progress.interpolate({
            inputRange: [0, 1],
            outputRange: [0, Math.sin(a) * distances[i]],
          });
        ...
        {angles.map((a, i) => {
          const tx = progress.interpolate({
            inputRange: [0, 1],
            outputRange: [0, Math.cos(a + Math.PI / 6) * (distances[i] * 0.7)],
          });
          const ty = progress.interpolate({
            inputRange: [0, 1],
            outputRange: [0, Math.sin(a + Math.PI / 6) * (distances[i] * 0.7)],
          });
          const scale = progress.interpolate({
            inputRange: [0, 0.4, 1],
            outputRange: [0.5, 1.0, 0.7],
          });
          const opacity = progress.interpolate({
            inputRange: [0, 0.2, 1],
